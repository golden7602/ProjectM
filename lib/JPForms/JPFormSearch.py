...
class Form_Search(QDialog):
    ...
    def __init__(self, tabinfo, base_sql, parent=None, flags=Qt.WindowFlags()):
        ...
        for fld in self.tabinfo.Fields:
            if fld.RowSource:
                fld.TypeCode = 9999
                tempde = myDe.JPDelegate_ComboBox(
                    self.tv, fld.RowSource)
    ...
    def accept(self):
        ...
        # 开始生成表达式
        lst = []
        # 当给定SQL时，由于要包裹一层子查询，所认表达式用title，否则用字段名列
        if self.BaseSQL:
            useFieldsColumn = 0
        else:
            useFieldsColumn = 1
        for i in range(rng):
            ...
            temp_exp = rd.Datas[4][1]['Sy'].format(fieldname=rd.Datas[3][1][useFieldsColumn],
                                                   value1=rd.Datas[5],
                                                   value2=rd.Datas[6])
        ...
        if self.BaseSQL:
            ...
            s = "accept方法生成了一个查询条件{}："
            s=s.format(self.Model.selectStatement(sql))
            logging.getLogger().debug(s)
            self.whereStringCreated.emit(sql)
        else:
            s = "accept方法生成了一个查询条件{}：".format(txt)
            logging.getLogger().debug(s)
            self.whereStringCreated.emit(txt)
        self.close()
